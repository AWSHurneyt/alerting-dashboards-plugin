...
export const getPerformanceModal = ({ edit, onClose, onSubmit, values }) => {
  console.info(`hurneyt getPerformanceModal::values = ${JSON.stringify(values, null, 4)}`);
  ...
  console.info(
    `hurneyt getPerformanceModal::monitorType = ${JSON.stringify(monitorType, null, 4)}`
  );
...
const QueryPerformance = ({ response, actions }) => {
  console.info(`hurneyt QueryPerformance::response = ${JSON.stringify(response, null, 4)}`);
  const monitorDuration = _.get(response, 'took', DEFAULT_EMPTY_DATA);
  const monitorDurationCallout = true;
  // TODO hurneyt uncomment after testing
  // monitorDuration >= RECOMMENDED_DURATION;

  // TODO hurneyt need to confirm the purpose of requestDuration.
  ...
  const requestDurationCallout = true;
  // requestDuration >= RECOMMENDED_DURATION;
