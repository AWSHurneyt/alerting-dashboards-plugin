...
import { EuiHealth, EuiLink, EuiText, EuiSpacer } from '@elastic/eui';
...
export class CrossClusterConfiguration extends Component {
  ...
  async getIndexes() {
    ...
    console.info(
      `hurneyt getRemoteIndexes::selectedClusters = ${JSON.stringify(selectedClusters, null, 4)}`
    );
    ...
    console.info(`hurneyt getRemoteIndexes::query = ${JSON.stringify(query, null, 4)}`);
    try {
      const response = await httpClient.get(`../api/alerting/remote/indexes`, { query: query });
      console.info(`hurneyt getAllRemoteIndexes::response = ${JSON.stringify(response, null, 4)}`);
      if (response.ok) {
  ...
  parseOptions = (clusterInfos = {}) => {
    ...
    // const indexOptions = []; //todo hurneyt
    ...
    console.info(`hurneyt parseOptions::indexes = ${JSON.stringify(indexes, null, 4)}`);
    ...
    Object.entries(clusterInfos).forEach(([clusterName, clusterInfo]) => {
      console.info(`hurneyt parseOptions::clusterName = ${JSON.stringify(clusterName, null, 4)}`);
      console.info(`hurneyt parseOptions::clusterInfo = ${JSON.stringify(clusterInfo, null, 4)}`);
      ...
      console.info(
        `hurneyt parseOptions::clusterOption = ${JSON.stringify(clusterOption, null, 4)}`
      );
      ...
      if (selectedClusters.some((option) => option.cluster === clusterName)) {
        const clusterIndexOptions =
          clusterInfo.indexes === undefined
            ...
            : Object.entries(clusterInfo.indexes).map(([_, indexInfo]) => {
                ...
                // TODO hurneyt: need to figure out how to handle wildcard patterns for index names.
                if (
                  !loadedInitialValues &&
        ...
        console.info(
          `hurneyt parseOptions::clusterIndexOptions = ${JSON.stringify(
            clusterIndexOptions,
            null,
            4
          )}`
        ...
        if (!categorizedIndexOptions[clusterInfo.cluster])
          categorizedIndexOptions[clusterInfo.cluster] = {
            label: clusterLabel,
            options: [],
          };
        ...
        // indexOptions.push({
        //   label: clusterLabel,
        //   options: _.orderBy(clusterIndexOptions, ['index'], ['asc']),
        // });
      }
    });

    console.info(
      `hurneyt parseOptions::categorizedIndexOptions = ${JSON.stringify(
        categorizedIndexOptions,
        null,
        4
      )}`
    );
    ...
    console.info(
      `hurneyt parseOptions::clusterOptions = ${JSON.stringify(clusterOptions, null, 4)}`
    );
    ...
    let outputState = {};
    console.info(
      `hurneyt parseOptions::selectedClusters = ${JSON.stringify(selectedClusters, null, 4)}`
    );
    console.info(
      `hurneyt parseOptions::selectedIndexes = ${JSON.stringify(selectedIndexes, null, 4)}`
    );
    if (!loadedInitialValues) {
    ...
    console.info(`hurneyt parseOptions::indexOptions = ${JSON.stringify(indexOptions, null, 4)}`);
    this.setState({
      ...outputState,
  ...
  renderClusterOption = (option) => {
    console.info(`hurneyt renderClusterOption::option = ${JSON.stringify(option, null, 4)}`);
  ...
  onClustersChange = (options = [], field, form) => {
    ...
    console.info(`hurneyt onClustersChange::options 1 = ${JSON.stringify(options, null, 4)}`);
    console.info(`hurneyt onClustersChange::field = ${JSON.stringify(field, null, 4)}`);
    ...
    if (!options.length) {
      const localClusterOption = clusterOptions.find((category) => category.label === 'Local')
        ?.options[0];
      console.info(
        `hurneyt onClustersChange::localClusterOption = ${JSON.stringify(
          localClusterOption,
          null,
          4
        )}`
    ...
    console.info(`hurneyt onClustersChange::options 2 = ${JSON.stringify(options, null, 4)}`);
    form.setFieldValue(
      field.name,
  ...
  renderClusterIndexesOption = (option) => {
    console.info(`hurneyt renderClusterIndexesOption::option = ${JSON.stringify(option, null, 4)}`);
    const { label, health } = option;
    return <EuiHealth color={HEALTH_TO_COLOR[health?.toLowerCase()]}>{label}</EuiHealth>;
  };
  ...
  renderSelectedClusterIndexesOptions = (options = []) => {
    console.info(
      `hurneyt renderSelectedClusterIndexesOptions::options = ${JSON.stringify(options, null, 4)}`
    );
  ...
  onIndexesChange = (options, field, form) => {
    console.info(`hurneyt onIndexesChange::options = ${JSON.stringify(options, null, 4)}`);
    console.info(`hurneyt onIndexesChange::field = ${JSON.stringify(field, null, 4)}`);
    try {
      console.info(`hurneyt onIndexesChange::form = ${JSON.stringify(form, null, 4)}`);
    } catch (e) {
      console.info(
        `hurneyt onIndexesChange::form KEYS = ${JSON.stringify(Object.keys(form), null, 4)}`
      );
    }
  ...
  onCreateOption = (value, field, form) => {
    ...
    console.info(`hurneyt onCreateOption::value = ${JSON.stringify(value, null, 4)}`);
    console.info(`hurneyt onCreateOption::field = ${JSON.stringify(field, null, 4)}`);
  ...
  render() {
    ...
    console.info(`hurneyt clusterOptions = ${JSON.stringify(clusterOptions, null, 4)}`);
    console.info(`hurneyt selectedClusters = ${JSON.stringify(selectedClusters, null, 4)}`);

    console.info(`hurneyt indexOptions = ${JSON.stringify(indexOptions, null, 4)}`);
