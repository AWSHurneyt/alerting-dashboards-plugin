...
const AnomalyDetectors = ({ values, detectorId, flyoutMode, landingDataSourceId }) => {
  const [detectorOptions, setDetectorOptions] = useState([]);
  const lastFetchedDataSourceId = useRef(undefined);

  useEffect(() => {
    const searchDetectors = async () => {
      const httpClient = getClient();
      try {
        const dataSourceQuery = landingDataSourceId
          ? { query: { dataSourceId: landingDataSourceId } }
          : getDataSourceQueryObj();

        const currentDataSourceId = dataSourceQuery?.query?.dataSourceId;
        if (lastFetchedDataSourceId.current === currentDataSourceId && detectorOptions.length) {
          return;
        }
        ...
        if (response.ok) {
          const options = response.detectors
          ...
          lastFetchedDataSourceId.current = currentDataSourceId;
        }
      } catch (err) {
        console.error(err);
      }
  ...
  return (
    <div style={{ maxWidth: flyoutMode ? '400px' : '390px' }}>
      {!flyoutMode && (
        <FormikComboBox
          name={'detectorId'}
          formRow
      ...
      {flyoutMode && (
        <FormikFieldText
          name="Detector"
          formRow
          rowProps={{ label: 'Detector' }}
          inputProps={{ value: detectorName, disabled: true }}
        />
      )}
    </div>
  );
